...
public class Modelo extends Conexion {
    ...
    public boolean hacerPedido(ArrayList nombres, ArrayList cant, String bar, String prov, boolean sql) {
        if (sql) {
            ...
            try {    //creamos la sentencia sql para insertar un pedido
    ...
    public boolean insertarProducto(String pro, String pre, String nom, boolean sql) {
        if (sql) {
            try {
                //creamos la sentencia sql para insertar un producto
            ...
            try {    //creamos la sentencia sqlite para insertar un producto
    ...
    public boolean actualizarProducto(String pro, String nom, String idpro, String pre, boolean sql) {
        if (sql) {
            try {
                // llamada al metodo para actualizar los campos de un producto
            ...
            try {    //creamos la sentencia sqlite para actualizar un producto
    ...
    public boolean borrarProducto(String idpro, boolean sql) {
        if (sql) {
            ...
            try {   
                ...
                ps.execute();
                ps.close();
                return true;     //si no salta ninguna excepcion se ha eliminado el producto
            } catch (SQLException ex) {
                System.err.println(ex.getMessage());
            }
            return false;
    ...
    public boolean insertaBar(String nom, String dom, String hor, String dias, String lic, boolean sql) {
        if (sql) {
            ...
            try {    
                ...
                ps.execute();
                ps.close();
                return true;     //si no salta ninguna excepcion se ha insertado el bar
            } catch (SQLException ex) {
                System.err.println(ex.getMessage());
            }
            return false;
    ...
    public boolean borrarBar(String ide, boolean sql) {
        if (sql) {
            ...
            try {    //creamos la sentencia sqlite para eliminar un bar
    ...
    public boolean crearEmple(String dn, String nom, String dom, boolean sql) {
        if (sql) {
            ...
            try {    
                ...
                ps.execute();
                ps.close();
                return true;     //si no salta ninguna excepcion se ha insertado el empleado
            } catch (SQLException ex) {
                System.err.println(ex.getMessage());
            }
            return false;
            ...
            try {    
                ...
                if (Integer.parseInt(cantlite) >= Integer.parseInt(cant) && Integer.parseInt(cant) > 0) {
                    ...
                    if (comprobarRecaudacion(bar)) {
                        p = "update recaudacion set total=total+" + cant + "*(select precio from inventario where codigo=" + cod + 
                                " and bar_idbar=" + bar + ") where bar_idbar=" + bar + " and fecha=date('now')";
                        ps = this.conexionLITE().prepareStatement(p);
                        ps.execute();
                        ps.close();
                    } else {
                        p = "insert into recaudacion (fecha, total, bar_idbar) VALUES (date('now'), " + cant + 
                                "*(select precio from inventario where codigo=" + cod + " and bar_idbar=" + bar + "), " + bar + ")";
                        ps = this.conexionLITE().prepareStatement(p);
                        ps.execute();
                        ps.close();
                    }
    ...
    public DefaultTableModel getBar(boolean sql) {
        ...
        if (sql) {
            ...
            String q = "select idbar, nombre, domicilio, fechaapertura as fecha_apertura,"
                    + " horario, diasapertura as dias_apertura, licenciafiscal as licencia_fiscal from bar order by idbar";
            try {
                PreparedStatement ps = this.conexionLITE().prepareStatement(q);
                ResultSet rs = ps.executeQuery();
                ResultSetMetaData rsmd = rs.getMetaData();
